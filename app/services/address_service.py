...
from app.exceptions import AddressOwnershipError
...
def create_address(data: dict) -> Address:
    ...
    try:
        ...
        db.session.rollback()
        raise Exception(f"Error creating address: {str(e)}")
...
def get_user_addresses(user_id) -> list[Address]:
    ...
    try:
        ...
        raise Exception(f"Error retrieving addresses for user ID {user_id}: {str(e)}")
...
def update_address(address_id: int, data: dict) -> Address:
    ...
    try:
        ...
        db.session.rollback()
        raise Exception(f"Error updating address ID {address_id}: {str(e)}")
...
def delete_address(address_id: int) -> None:
    ...
    try:
        ...
        db.session.rollback()
        raise Exception(f"Error deleting address ID {address_id}: {str(e)}")